...
class Spidey:
    ...
    async def add_premium(user_id, time_value, time_unit):
        ...
        await collection.update_one(
            {"user_id": user_id}, {"$set": premium_data}, upsert=True
        )
...
async def check_user_plan(user_id):
    ...
    if user:
        expiration_timestamp = user["expiration_timestamp"]
        # Convert expiration timestamp to a timezone-aware datetime object in IST
        expiration_time = datetime.fromisoformat(expiration_timestamp).astimezone(
...
async def has_premium_access(user_id):
    ...
    if not expiration:
        return False
    expiration_time = datetime.fromisoformat(expiration).astimezone(
        timezone("Asia/Kolkata")
    )
